...
const _SimpleList = styled.div`
  ...
  .articleLine {
    ...
    @media (min-width: 1000px){
      font-size: 1.2em;
      grid-template-columns: minmax(min-content, 250px) auto auto;
      .descrizione{
        order: 0;
        padding: 0 1em;
        margin-top: 0;
    ...
    @media (min-width: 1400px){
      font-size: 1.3em;
      grid-template-columns: minmax(min-content, 350px) auto auto;
    }
  }
        ...
        <div key={d.id} className='catWrapper' >
          ...
          <div className='categoryContent'>
            {d.articoli.map(a => {
              ...
              return (
                <HashLink disabled={a.immagine[0] === undefined} className={lineClassName.join(' ')} key={a.id} id={`pic_${a.id}`}>
                  <p className='titolo'>
                    {a.titolo}
                  </p>
                  ...
                  <MultiPrice className='prezzo' priceList={a.multiPrezzo} />
                </HashLink>
              )
            })
            }
